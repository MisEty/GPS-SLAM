...
void UIEngine::glutDisplayFunction()
{
    ...
    // 根据当前的out image type把渲染结果放在out image 0里，对应于显示屏左边的图像
    ...
    // 始终把outimage type 1和2的图片放在out image 1和2里，对应于显示屏左上和右上的图像
                          ...
                          ITMMainEngine *mainEngine)
{
    ...
    winSize.x = (int)(1.5f * (float)(GetDepthSize().x));
    winSize.y = GetDepthSize().y + textHeight;
    ...
    for (int w = 0; w < NUM_WIN; w++)
        outImage[w] = new ITMUChar4Image(GetDepthSize(), true, allocateGPU);
    ...
    saveImage = new ITMUChar4Image(GetDepthSize(), true, false);
...
void UIEngine::ProcessFrame()
{
    ...
    std::cout << "recontruction frame time: " << duration.count() << " s" << std::endl;
